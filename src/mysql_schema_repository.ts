...
class MySqlSchemaRepository implements SchemaInterface {
    ...
    async hasColumn(tableName: string, columnName: string): Promise<boolean>{
        ...
        try {
            let result = await mysql.query(`SELECT * FROM information_schema.COLUMNS WHERE
            TABLE_SCHEMA = ? AND TABLE_NAME = ? AND COLUMN_NAME = ?`, [dbName, tableName, columnName])
            if(result.length > 0){
                return true;
            }
            return false
    ...
    // create(tableName: string, callback: (builder: BuilderInterface) => void): Promise<boolean>
