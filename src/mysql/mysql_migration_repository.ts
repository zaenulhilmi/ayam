...
class MysqlMigrationRepository implements MigrationRepositoryInterface{
    ...
    create(): MigrationRepositoryInterface {
        this.query = `create table migrations (id bigint primary key auto_increment, file_name varchar(255) not null, step int not null, created_at timestamp default current_timestamp, updated_at timestamp default current_timestamp);`
        return this
    }
    ...
    removeAllLastStep(): MigrationRepositoryInterface {
        this.query = `delete t.* from migrations t inner join (select max(step) max_step FROM migrations) tmax ON t.step = tmax.max_step;`
        return this
    }
