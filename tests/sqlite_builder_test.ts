...
Deno.test('create a table using id()', async () => {
    ...
    table.id()
    assertEquals(table.toSql(), 'create table users (id integer not null primary key autoincrement);')
})
...
Deno.test('create a table and a field of timestamp', async () => {
    ...
    assertEquals(table.toSql(), 'create table users (subscribed_at text not null);')
})
