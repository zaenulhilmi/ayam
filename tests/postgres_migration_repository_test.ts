...
Deno.test(description('create migrations table'), async () => {
    ...
    repo.create()
    assertEquals(repo.toSql(), `create table migrations (id bigserial primary key, file_name varchar(255) not null, step int not null, created_at timestamp default current_timestamp, updated_at timestamp default current_timestamp);`)
})

Deno.test(description("insert into migrations table"), async () => {
    let repo: MigrationRepositoryInterface = new PostgresMigrationRepository()
    repo.insert('lalala_3.ts', 2)
    assertEquals(repo.toSql(), `insert into migrations (file_name, step) values ('lalala_3.ts', '2');`)
})
...
Deno.test(description("delete last migration step"), () => {
    ...
    assertEquals(repo.toSql(), `delete from migrations where step in (select max(step) from migrations);`)
})