...
Deno.test('postgres find table', async () => {
    ...
    assertEquals(repo.toSql(), `select * from information_schema.tables where table_schema='public' and table_name='users';`)
})

Deno.test('postgres find table column', async () => {
    let repo: RepositoryInterface  = new PostgresRepository();
    repo.findTableColumn('users', 'name')
    assertEquals(repo.toSql(), `select * from information_schema.columns where table_schema='public' and table_name='users' and column_name='name';`)
})
