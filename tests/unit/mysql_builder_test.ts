...
Deno.test("create table with a field of timestamp", async () => {
  ...
  assertEquals(
    table.toSql(),
    `create table users (published_at timestamp not null);`,
  );
});
...
Deno.test("create table with a field of default timestamps", async () => {
  ...
  assertEquals(
    table.toSql(),
    `create table users (created_at timestamp not null, updated_at timestamp not null);`,
  );
});
...
Deno.test("create table with a field of integer unsigned", async () => {
  ...
  assertEquals(
    table.toSql(),
    `create table users (age int unsigned not null);`,
  );
});
...
Deno.test("create table with a field of integer unsigned default value", async () => {
  ...
  assertEquals(
    table.toSql(),
    `create table users (age int not null default '4');`,
  );
});
